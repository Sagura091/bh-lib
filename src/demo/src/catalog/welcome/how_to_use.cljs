(ns demo.src.catalog.welcome.how-to-use
  (:require [demo.src.catalog.technologies.overview.overview :as o]))


(defn overview []
  [o/overview
   "How to Use this demo.src.catalog"

   "This demo.src.catalog of UI Components serves many different purposes, for many different kinds of people, who
   are trying to solve many different kinds of problems. Specifically, we designed this demo.src.catalog to support the needs of:
...
For Developers, this demo.src.catalog shows all the UI software components _in use_. You can see how the component works, what it looks like,
...
Much like Developers, Designers can use the demo.src.catalog to see the whole range of UI Components available for them to use
...
Product Owners can use the demo.src.catalog to get a _sense of the possible_. Having the entire collection of components at your
...
End Users can use this demo.src.catalog in much the same way as the Product Owners; as a way to see what is possible. Many times, users
find it difficult to express their needs to technical people. Having a demo.src.catalog that everyone shares can help make it easier to
